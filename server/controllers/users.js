...
export const addRemoveFriend = async (req, res) => {
  try {
    ...
    // Find both the user and the friend using their IDs
    const user = await User.findById(id);
    const friend = await User.findById(friendId);

    // Check if either user or friend is an admin
    if (user.isAdmin || friend.isAdmin) {
      return res.status(403).json({ message: "Adding or removing an admin as a friend is not allowed." });
    }

    // If the user is already friends with the friendId, remove them
    if (user.friends.includes(friendId)) {
      user.friends = user.friends.filter((friend) => friend.toString() !== friendId.toString());
      friend.friends = friend.friends.filter((userId) => userId.toString() !== id.toString());
    } else {
      // Otherwise, add them as friends
      user.friends.push(friendId);
      friend.friends.push(id);
    }

    // Save the changes to both the user and the friend
    await user.save();
    await friend.save();

    // Fetch updated list of friends and format the response
    const updatedFriends = await User.find({ _id: { $in: user.friends } });
    const formattedFriends = updatedFriends.map(
      ({ _id, firstName, lastName, occupation, location, picturePath }) => {
        return { _id, firstName, lastName, occupation, location, picturePath };
